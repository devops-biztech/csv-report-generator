...
| linked_accounts | id, **userId**, **provider**, tellerAccountId, accessToken, institutionName, **syncEnabled**, syncStartDate, **lastSuccessfulSyncAt**, **balanceCached**, **balanceFetchedAt** | provider='teller'\|'simplefin'; tellerAccountId/accessToken hold the provider's account ID / access URL. lastSyncedAt = last attempt (lazy-sync claim bumps on failure too); lastSuccessfulSyncAt = success only; balanceCached/balanceFetchedAt back the 4h live-balance gate |
...
- **Lazy auto-sync** (`lib/bankSync.ts` `lazySyncIfStale`, added Jul 16 2026; interval bumped 1h→**4h** Jul 29 2026 to cut SimpleFIN's rotating-IP "new IP" emails): GET `/api/bank/sync` auto-syncs any account with `lastSyncedAt` older than `SYNC_STALENESS_MS` (4h) before returning uncategorized txns. The claim is an atomic UPDATE that bumps `lastSyncedAt` up front — concurrent requests can't double-sync, and failures retry next window (not per-request), keeping usage within SimpleFIN's ~24 req/day. **Live balances** (GET `/api/bank/balances`) are gated the same way (`BALANCE_STALENESS_MS` 4h): a `balanceCached`/`balanceFetchedAt` pair on `linked_accounts` serves the last balance between fetches so app opens / Settings visits / iOS pull-to-refresh don't hit SimpleFIN live more than once/4h. No cron: chosen over Vercel Cron (hourly needs Pro) and GitHub Actions — syncs only while the app is in use. Core sync logic lives in `syncSimplefinAccounts()`, shared with POST. **Web manual sync buttons removed Jul 16 2026** (BudgetSummary Sync + settings Sync All) — lazy sync is the only web trigger; POST `/api/bank/sync` remains for iOS manual sync. Web sync-error toasts went with the buttons; lazy-sync errors are server logs only for now. New-tab empty state shows "Last synced X ago" (BudgetSummary fetches accounts *after* the uncategorized fetch so the timestamp reflects the lazy sync that just ran)
